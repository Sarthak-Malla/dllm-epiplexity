...
def test_samplers():
    ...
    greedy_config = MDLMSamplerConfig(steps=12, max_new_tokens=96, remasking="low_confidence")
    oracle_config = OracleEpiplexitySamplerConfig(steps=12, max_new_tokens=96, oracle_candidate_strategy="mixed")
    ...
    for idx, (prompt, expected_answer) in enumerate(TEST_PROMPTS):
        ...
        # Convert to list of 1D tensors or lists for the sampler
        inputs_list = [input_ids[0]]

        print("Running Greedy MDLMSampler...")
        with torch.no_grad():
            greedy_out = greedy_sampler.sample(inputs_list, config=greedy_config)
        greedy_ans = tokenizer.decode(greedy_out[0, input_ids.shape[1]:], skip_special_tokens=True)
        
        print("Running Oracle Epiplexity Sampler...")
        with torch.no_grad():
            oracle_out = oracle_sampler.sample(inputs_list, config=oracle_config)
        oracle_ans = tokenizer.decode(oracle_out[0, input_ids.shape[1]:], skip_special_tokens=True)
